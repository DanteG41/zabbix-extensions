...
DEPEND=">=net-analyzer/zabbix-2.0.0
		...
		hwraid_fusion_mpt? ( sys-block/mpt-status )
...
src_install() {
	...
	if use postgres; then
		...
		doexe \
			...
			files/postgresql/scripts/pgsql.replication.slots.discovery.sh \
			files/postgresql/scripts/pgsql.replication.slots.stat.sh \
			files/postgresql/scripts/pgsql.replication.subscriptions.discovery.sh \
			files/postgresql/scripts/pgsql.replication.subscriptions.stat.sh \
			...
			files/postgresql/scripts/pgsql.notification_queue.sh
	fi
